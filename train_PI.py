...
# policy = MLP(env.spec, hidden_sizes=data['policy_size'], seed=data['seed'], device = Device) # device = Device -> FCN.to(device)
...
if data['algorithm'] != 'NPG':
    ...
    bc_agent = BehaviorCloning(demo_paths, policy=policy, epochs=data['bc_epochs'], batch_size=data['bc_batch_size'],
                  lr=data['bc_learn_rate'], loss_type='MSE', set_transforms=False)
    # bc_agent = BehaviorCloning(demo_paths, policy=policy, epochs=data['bc_epochs'], batch_size=data['bc_batch_size'],
    #               lr=data['bc_learn_rate'], loss_type='MSE', set_transforms=False, device = Device)
